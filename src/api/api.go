...
import (
	...
	"github.com/zeshanwd/go-rest-api/src/helpers"
...
func SearchTodo(w http.ResponseWriter, req *http.Request) {
	vars := mux.Vars(req)
	id := vars["id"]
...
func ListTodos(w http.ResponseWriter, req *http.Request) {
...
func UpdateTodo(w http.ResponseWriter, req *http.Request) {
	...
	if success != true {
		http.Error(w, "could not decode body to update", http.StatusBadRequest)
		return
	}
	...
	if !helpers.IsValidDescription(bodyTodo.Description) {
		data.Success = false
		data.Errors = append(data.Errors, "Invalid Description to Update")
